...
const collection = global.collection =  new class{
		...
		newUser(ws,req,connection_ip){
				...
				this.indexing.forEach((key)=>{        
						this.changeUserdata(user,key,user[key])
				 })
			 ...
			 switch(opcode){
					 ...
					 case 100://Chat Message
							 ...
							 if(user.shadownick !== ''){
									...
									if(collection.by.tokenTag.hasOwnProperty(user.tokenTag)){
										collection.by.tokenTag[user.tokenTag].forEach(targetClient=>{
											if(targetClient == user || targetClient.ip === user.ip) return targetClient.send(message);
											targetClient.send(fakeMessage)
										})
									} 
				 ...
				 default:  
					 ...
					 if(typeof reader == "object" && message.readUInt8 && message.byteLength>2 && message.readUInt8(2)==101){//msgpack protocol, array[4] is == "e"
