 ...
 (function () {
  function initNetwork(container, opts = {}) {
    ...
    const computeCount = () => {
      ...
      // further reduce divisor to increase particle density significantly
      const divisor = opts.light ? 3000 : 1200;
      // raise maximum cap so large headers get densely filled
      return Math.max(30, Math.min(600, Math.floor(area / divisor)));
    };
    ...
    function draw() {
      const w = container.clientWidth;
      const h = container.clientHeight;
      // limpar em unidades CSS (dividindo por dpr) para evitar artefatos
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (d2 <= maxDist2) {
            const alpha = 0.85 * (1 - Math.sqrt(d2) / maxDist);
            // blue lines
            ctx.strokeStyle = 'rgba(0,150,255,' + alpha.toFixed(3) + ')';
            ctx.lineWidth = 1.0;
            ctx.beginPath();
            ctx.moveTo(A.x, A.y);
            ctx.lineTo(B.x, B.y);
