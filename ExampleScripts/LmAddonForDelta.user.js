...
// Detects agar.io's skin editor canvas and injects image upload controls.
// Supports: URL input, file upload, drag & drop.
// Original concept by New Jack 🕹️, integrated into LM addon by Jimboy3100.
// ==========================================================================
(function () {
    'use strict';
    const win = typeof unsafeWindow !== 'undefined' ? unsafeWindow : window;
    ...
    // ── Observe DOM for skin editor open/close ──
    const skinObserver = new MutationObserver(mutations => {
    ...
    function startSkinObserver() {
        if (document.body) {
            skinObserver.observe(document.body, { childList: true, subtree: true });
        } else {
            setTimeout(startSkinObserver, 100);
        }
    }

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', startSkinObserver);
    } else {
        startSkinObserver();
    }
    ...
    function initSkinUploader() {
        ...
        console.log((win.LOG_TAG || '[LM] ') + '✓ Skin image uploader injected');
    }
    ...
    function drawSkinToCanvas(img) {
        ...
        try { if (win.drawApp && typeof win.drawApp.render === 'function') win.drawApp.render(true); } catch (e) {}
    }
    ...
    function makeSkinDraggable(el, handle) {
        ...
        handle.onmousedown = (e) => {
            ...
            document.onmousemove = (e) => {
                e.preventDefault();
                el.style.top = (el.offsetTop - (y2 - e.clientY)) + 'px';
                el.style.left = (el.offsetLeft - (x2 - e.clientX)) + 'px';
                el.style.right = 'auto';
                x2 = e.clientX; y2 = e.clientY;
            };
        };
