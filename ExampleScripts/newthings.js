function SpecialDeals() {
    ...
    if ($("#UserProfileUID1").text() != null) {
        $('#helloContainer').after('<div class="modal fade in" id="specialShopModal" aria-hidden="false" style="display: block;">' +
            ...
            '<p class="alert-warning text-center">' + Premadeletter116 + '<br><span class="alert-success" id="exp-uid">' + $('#UserProfileUID1').text() + '</span> <font color="red" onclick=copy($("#UserProfileUID1").text());><b><u>'+Premadeletter114+' UID</u></b></font><br>*UID ' + Premadeletter115 + '</p>' +
            '</div>' +
            '</div>' +
            '</div>' +
            '</div>');
