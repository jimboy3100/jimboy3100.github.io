...
console.log('[SERVER] Running bots version', user.version)
...
new WebSocket.Server({
    ...
    ws.on('message', buffer => {
        ...
        switch(reader.readUint8()){
            ...
            case 2:
                for(const bot of userBots){
                    if(bot.isAlive && bot.followMouse && !stoppingBots && !bots.ai) bot.send(Buffer.from([17])) //sendEject
					//if(bot.isAlive && bot.followMouse && !stoppingBots && !bots.ai) bot.sendEject()
                }
                break
            case 3:
                for(const bot of userBots){
                    if(bot.isAlive && bot.followMouse && !stoppingBots && !bots.ai) bot.send(Buffer.from([21])) //sendSplit
