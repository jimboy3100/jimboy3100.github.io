...
for (var i=1;i<window.RecordPlayers.length;i++){
	...
	'SID: ' + window.RecordPlayers[i].socialid + ' <img src=' + window.RecordPlayers[i].icon + ' style="width: 40px; height:40px;">'
            ...
            $('#UserLeaguesInfoModal').remove();
            $('.modal-backdrop').remove();
            $('#helloContainer').after('<div class="modal fade in" id="UserLeaguesInfoModal" aria-hidden="false" style="display: block;">' +
                ...
                '<div id="CloseUserLeaguesInfo2" class="modal-header"><button id="CloseUserLeaguesInfo" type="button" class="close" data-dismiss="modal" title="Close"><span aria-hidden="true">×</span><span class="sr-only">' + Premadeletter113 + '</span></button> <button id="FAQUserLeaguesInfo" type="button" class="close" title="Help"><span aria-hidden="true">?</span><span class="sr-only">' + Premadeletter113 + '</span></button>' +
                '<h4 class="modal-title" style="font-family: Roboto Condensed, sans-serif">' + 'User Leagues Info' + '</h4>' +
                '</div>' +
				
				'<div id="UserLeaguesInfo3" style="overflow-y: auto; max-height: 70vh;">' +
   ...
   '</div>' +
	...
	'</div>' +				
            ...
            $("#CloseUserLeaguesInfo").off('click').on('click', function(e) {
                if (e) e.stopPropagation();
                $("#UserLeaguesInfoModal").remove();
                $(".modal-backdrop").remove();
                $("body").removeClass("modal-open");
            });
            $("#FAQUserLeaguesInfo").off('click').on('click', function(e) {
                if (e) e.stopPropagation();
